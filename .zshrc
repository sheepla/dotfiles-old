...
function _gosrc-fzf() {
    local dir="$(find ~/go/src -type d -not -name ".git" -maxdepth 3 | fzf --reverse --height="50%" --preview="ls -AF --color=always {1}")"
    if [ -n "~/go/src/${dir}" ]; then
        cd "${dir}"
        zle accept-line
        zle reset-prompt
    else
...
zle -N _gosrc-fzf
bindkey "^g^o" _gosrc-fzf

function _dein-fzf() {
    local dir="$(find ~/.cache/dein/repos -type d -not -name ".git" -maxdepth 3 | fzf --reverse --height="50%" --preview="ls -AF --color=always {1}")"
    if [ -n "~/.cache/dein/${dir}" ]; then
        cd "${dir}"
        zle accept-line
        zle reset-prompt
    else
